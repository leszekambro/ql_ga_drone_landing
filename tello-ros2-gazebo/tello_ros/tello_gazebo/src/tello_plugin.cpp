...
namespace tello_gazebo
{
  ...
  const double MAX_XY_A = 8; 
  const double MAX_Z_A = 4; 
  const double MAX_ANG_A = M_PI;

  const double TAKEOFF_Z = 1;       // Takeoff target z position
  const double TAKEOFF_Z_V = 0.8;     // Takeoff target z velocity

  const double LAND_Z = 0.1;          // Land target z position
  const double LAND_Z_V = -0.5;       // Land target z velocity

  const double BATTERY_DURATION = 999999999;  // Battery duration in seconds
  ...
  class TelloPlugin : public gazebo::ModelPlugin
  {
    ...
    double battery_duration_{BATTERY_DURATION};
  ...
  public:
    ...
    void Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
    {
      ...
      if (sdf->HasElement("battery_duration")) {
        battery_duration_ = sdf->GetElement("center_of_mass")->Get<double>();
      }
    ...
    void OnUpdate(const gazebo::common::UpdateInfo &info)
    {
      ...
      if (flight_state_ != FlightState::landed) {
        ...
        float rot_x = lin_ubar.X()*0.1;
        if (rot_x > 0.1) rot_x = 0.1;
        if (rot_x < -0.1) rot_x = -0.1;
        float rot_y = lin_ubar.Y()*0.1;
        if (rot_y > 0.1) rot_y = 0.1;
        if (rot_y < -0.1) rot_y = -0.1;
...
#pragma clang diagnostic pop
    ...
    void spin_10Hz()
    {
      ...
      // Simulate a battery
      int battery_percent = static_cast<double>((battery_duration_ - ros_time.seconds()) / battery_duration_ * 100);
